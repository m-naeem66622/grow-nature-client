...
const PlantSwapList = () => {
  ...
  const [filter, setFilter] = useState(searchParams.get("tab") || "");
  ...
  const [plantSwap, setPlantSwap] = useState({});
  const deleteModalRef = useRef(null);

  const title = "My Plant Swaps";

  const fetchPlantSwaps = async (fetchFor = "user") => {
    const query = queryParser({ page, limit });
    try {
      const response = await axios.get(
        `${PLANT_SWAPS_URL}/${fetchFor}/me?${query}`,
        {
          headers: {
        ...
        .then((response) => {
          ...
          if (pagination.currentPage < pagination.totalPages) {
            ...
            setPagination((prevState) => {
              const newTotalProducts = prevState.totalPlantSwaps - 1;
              return {
                ...prevState,
                totalPlantSwaps: newTotalProducts,
                currentPlantSwaps: prevState.currentPlantSwaps - 1,
                totalPages: Math.ceil(newTotalProducts / limit),
              };
            });

            if (pagination.currentPlantSwaps === 1) {
  ...
  const paginationHeader = (
    <div className="flex justify-center items-center mb-4">
      ...
      {/* Filter Element */}
      <div htmlFor="filter" className="label ml-4 mr-2">
      ...
      <select
        ...
        onChange={(e) => {
          ...
          fetchPlantSwaps(e.target.value || "user");
        }}
      >
        <option value="">My Swaps</option>
        <option value="partner">Partnered Swaps</option>
      </select>
    </div>
  );
  ...
  return (
    <>
      <ConfirmationModal
        ...
        onConfirm={() => handleDelete(plantSwap._id)}
        variant="delete"
      >
        <h3 className="font-bold text-lg">Delete Confirmation</h3>
        <p className="py-4">Are you sure you want to delete this plant swap?</p>
      </ConfirmationModal>
      <Container>
        <div className="flex justify-between">
          <Heading className="text-4xl font-bold mb-8 mt-4">{title}</Heading>
          ...
          {data.map((plantSwap) => (
            ...
            >
              <div className="p-4 flex flex-col justify-between h-full">
                    ...
                    {plantSwap.swapPartner ? (
                      <>
                        ...
                        <ul className="list-disc ml-8">
                          <li>
                            {plantSwap.swapPartner.firstName}{" "}
                            {plantSwap.swapPartner.lastName}
                          </li>
                          <li>Email: {plantSwap.swapPartner.email}</li>
                          <li>Phone: {plantSwap.swapPartner.phoneNumber}</li>
                        </ul>
                      </>
                    ) : (
                ...
                {/* Wanted Plants  */}
                <div className="flex justify-between text-neutral"></div>
                {filter !== "partner" && (
                  <div className="flex justify-end gap-x-2">
                    ...
                    <Link
                      ...
                      to={`/user/plant-swaps/${plantSwap._id}/edit`}
                    >
                      Edit
                    </Link>
                  </div>
